...
class CMTT_AMP {
	...
	public static function init() {
		 ...
		 */
		add_action( 'amp_post_template_css', [ __CLASS__, 'amp_custom_css' ], 11 );
		add_filter( 'cmtt_pre_item_description_content', array( __CLASS__, 'outputSimpleTooltipContent' ), 50, 4 );
		...
		add_filter( 'cmtt_pre_item_description_content', array( __CLASS__, 'convertBindAttributes' ), 100 );
		...
		add_filter( 'cmtt_glossary_index_before_listnav_content', array( __CLASS__, 'outputBeforeListnav' ), 20, 3 );
	}
	...
	public static function outputBeforeListnav( $content, $shortcodeAtts, $glossary_query ) {
		...
		if ( self::is_amp_endpoint() ) :
			ob_start();
			$clearLink = get_permalink( $wp_query->post->ID ) . '?amp';
			?>
			...
			$content     = $new_content . $content . '</form>';
		endif;
	 ...
	 */
	public static function outputSimpleTooltipContent( $postItemTitleContent, $glossaryItem, $glossaryIndexStyle, $id ) {
		$tooltip = '';
		...
		if ( CMTT_AMP::is_amp_endpoint() ) {
			...
			<span id="tt"
			      ...
			      aria-hidden="true"
			      [aria-hidden]="cmindsState.visibleTooltip!='tooltip-<?php echo $glossaryItem->ID . $id; ?>'"
			      hidden="true"
			      [hidden]="cmindsState.visibleTooltip!='tooltip-<?php echo $glossaryItem->ID . $id; ?>'"
	...
	public static function amp_custom_css() {
		...
		?>
		#tt.amp-tooltip[aria-hidden="true"] {
		display: none;
		}
		#tt.amp-tooltip[aria-hidden="false"] {
		display: block;
		}
